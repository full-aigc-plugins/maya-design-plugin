...
- 需要导出视频 → 用 `codex-maya-export-preview`（报错在导出阶段产生时，最终解释仍回到本技能）
- 需要盘点场景 → 用 `codex-maya-inspect`
